...
void PolyLineToRas::markOutlet( int x, int y, int streamID, int segment, int endSegmentX, int endSegmentY, float * demArray, DIRECTION d )
{	

	if( x != endSegmentX || y != endSegmentY )
	{	if( x >= 0 && x < ncols && y >= 0 && y < nrows )
		{	if( demArray[ indexData( x, y ) ] > float_nodata_value )  //dgt changed != to >
				{	streamOutletX = x;
					streamOutletY = y;
					return;
				}	
...
bool PolyLineToRas::findValidDemValue( int x, int y, int streamID, int segment, int & newX, int & newY, int & newSegment, DIRECTION d, float * demArray )
{	
	if( x >= 0 && x < ncols && y >= 0 && y < nrows )
	{	if( demArray[ indexData( x, y ) ] > float_nodata_value )  //dgt changed != to >
			{	newX = x;
...
bool PolyLineToRas::createFlow( const char * idColumnName, const char * downPointer, const char * dbfPath, float * demArray, const char * streamOutletID )
{	
	int percent = 0;
...
/* //dgt	ifstream dem( demFile );
  ...
  */
	...
	float * demArray = new float[ totalSize ];
	{
	char elevfile[MAXLN];
	float **elev,dx,dy;
	int err;
...
//dgt	for( int s = 0; s < totalSize; s++ )
//dgt		demArray[s] = nodata_value;
...
void PolyLineToRas::writeFlowFile( CString outfile, float * demArray )
{	
	char fname[MAXLN];
	float dx=cellsize;
	float dy=cellsize;
