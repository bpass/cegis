/* Program import channel network links into arc  */
...
void main(int argc, char **argv)
 {
  ...
  if(argc < 2)
  {
     ...
     printf("\narclinks <filename_prefix> [-i (to call import)] \n");
  ...
  else
  {
    ...
    strcat(outfile,"li.e00");
	strcpy(outfile2,"");
	strncat(outfile2,argv[1],nmain);
    strcat(outfile2,"li");
  }

	arclinks(coordfile,treefile,outfile,&ilink,&amin);
	if(avrun)
	{
	...
	if(idel)
	{
		printf("Press enter after import to continue\n");
		scanf("%c",&outfile2[0]);
