...
public 	UTM(double gctpParams[], ProjUnit units, ProjDatum dat, int zone)
{
...
protected 	void _loadFromParams() throws ProjException
{
    ...
    if(m_zone == 0)
    {
        ...
        m_zone = Util.calc_utm_zone(lon * Constants.R2D);
        if(lat < 0)
            m_zone *= -1;
    }
}
...
protected 	void _init() throws ProjException
{
    ...
    if ((Math.abs(m_zone) < 1) || (Math.abs(m_zone) > 60))
    {
        throw(new ProjException(11, "UTM._init()"));
       
    }

    m_center.lon = ((6 * Math.abs(m_zone)) - 183) * Constants.D2R;
...
protected void _forward(GeoPoint p) throws ProjException
{
    ...
    if (m_ind != 0)
    {
        ...
        {
            throw(new ProjException(93, "UTM._forward()"));
...
protected void _inverse(CoordPoint p) throws ProjException
{
    ...
    for (i=0;;i++)
    {
        ...
        if (i >= max_iter) 
        { 
            throw(new ProjException(95, "UTM._inverse()"));
