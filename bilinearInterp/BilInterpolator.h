...
private:
	...
	std::vector< std::vector<double> > m_results; //Interpolated values for each input pixel for each layer
	FILE* m_imageFilePtr; //raster image file
	std::string m_imageFileName;
	double doInterpolation(const Point& p, int band); //Do an interpolation on a pixel and a certain point
