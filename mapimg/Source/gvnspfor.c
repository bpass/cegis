// $Id: gvnspfor.c,v 1.1 2005/07/12 16:21:04 rbuehler Exp $
...
/* Forward equations
  -----------------*/
dlon = adjust_lon((double)(lon - lon_center));
gctp_sincos(lat,&sinphi,&cosphi);
coslon = cos(dlon);

//R = radius of sphere at the scale of the map
//dlon =  (lon = longitude east of Greenwich  - lon_cneter = longitude east of Greenwich used as center )
//phi = north latitude (- if south)
//p = distance of point of perspective from the center of the earth divided by R
//    (- if below surface of earth
//g = cos(  great-circle distance, as an arc of a circle  )
//ksp = intermediate value
