...
bool mapimg::downSampleImg( const RasterInfo &input, RasterInfo &output, int maxDimension, QWidget *parent )
{
	...
	if( tmp != QString::null )
		...
		output.setFileName( QDir::currentPath().append("/temp_small.img") );

	mapimg::downSizeProjection(output, maxDimension);

	QString dtype = "";
	dtype += input.fullDataType();
...
double mapimg::calcFillValue( const RasterInfo &input )
{
	...
	{   qint32 data = 0;
	return IMGIO<qint32>::get_max_value( input, data );   }
	else if( dtype == "Unsigned 32 Bit Integer" )
	{   quint32 data = 0;
	return IMGIO<quint32>::get_max_value( input, data );   }
	else if( dtype == "Signed 16 Bit Integer" )
	{   qint16 data = 0;
	return IMGIO<qint16>::get_max_value( input, data );   }
	else if( dtype == "Unsigned 16 Bit Integer" )
	{   quint16 data = 0;
	return IMGIO<quint16>::get_max_value( input, data );   }
	else if( dtype == "Signed 8 Bit Integer" )
	{    qint8 data = 0;
	return IMGIO<qint8>::get_max_value( input, data );   }
	else //( dtype == "Unsigned 8 Bit Integer" )
	{  quint8 data = 0;
	return IMGIO<quint8>::get_max_value( input, data );   }
}

bool mapimg::reproject( const RasterInfo &input, const RasterInfo &output, const ResampleInfo &resample, QWidget *parent )
{
	QString dtype = "";
	dtype += output.fullDataType();
