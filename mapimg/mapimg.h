// $Id: mapimg.h,v 1.11 2005/02/25 18:37:49 jtrent Exp $
...
//        qDebug( "Here 2" );
        ...
        *((type*)values+left) = pivot_value;
        ...
        if( left < pivot_value )
            mapimg::quicksort<type>( values, size, left, pivot_value-1 );

        qDebug( QString("Here 3, %1 %2 %3").arg( size ).arg( pivot_value+1 ).arg( right ).ascii() );

        if( right > pivot_value )
        {
            qDebug( "Here 3a" );
            mapimg::quicksort<type>( values, size, pivot_value+1, right );
            qDebug( "Here 3b" );
        }
