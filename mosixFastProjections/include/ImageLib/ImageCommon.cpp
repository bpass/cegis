...
// $Id: ImageCommon.cpp,v 1.1 2005/03/11 23:59:08 mschisler Exp $
...
{
  // substituted by Matt Zykan joo-lie 2004
  // (way way way faster, but could be less crappy, ah well, give/take)
  char * sbuf = new char[s.length() + 1];
  strncpy(sbuf, s.c_str(), s.length());
  sbuf[s.length()] = '\0'; // oh lordy the paranoia
  char * testkey = strtok(sbuf, " ");
  if(testkey == NULL)
    return false;
  bool match = (strcmp(testkey, inchar) == 0);
  delete[] sbuf;
  return match;
