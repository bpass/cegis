...
throw(ProjectorException)
{
  ...
  //ADDED to fix memory passing problems -CBB 8/1/01
  //clone the projection
  toprojection = inoutproject->clone();
...
void Projector::setnewheight(const long int newnewheight) throw()
{
  newheight = newnewheight;
}

void Projector::setnewwidth(const long int newnewwidth) throw()
{
...
std::string Projector::getInputFile() const throw()
{return infilename;}

MathLib::Point Projector::getOutputScale() const throw()
{return newscale;}

bool Projector::getsamescale() const throw()
{return samescale;}

DRect Projector::getoutRect() const throw()
{return outRect;}

long int Projector::getnewheight() const throw()
{return newheight;}

long int Projector::getnewwidth() const throw()
{return newwidth;}

/////////////////////////////////////////////////////////

void Projector::dogetExtents() throw()
{
  PmeshLib::ProjectionMesh * pmesh(0);
  pmesh = setupForwardPmesh();
  if(pmesh != NULL)
  {
    getExtents(pmesh);
  }
...
throw(ProjectorException)
{
  ...
  try
  {
    ...
    {
      char msgbuf[64];
      snprintf(msgbuf, 64, "Reprojecting %ld lines.", newheight);
      progress->init(
          msgbuf,
          NULL,
          "Done.",
...
void Projector::setupInput(std::string & ininfile) throw(ProjectorException)
{
  ...
  try
  {
    //check for preexisting input file
    if(infile)
    {
      delete infile;
      infile = NULL;
...
{
  (void)inoutfile; // kill unused variable warning
