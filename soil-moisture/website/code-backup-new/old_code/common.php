...
function oracleEscape(&$query, $searching) {
	...
	function printNewsForm() {
		...
		print "<div class=\"spacer\">Add Project News</div>
			...
			<tr><td>HTML Tags Allowed:  &lt;a&gt;, &lt;p&gt;,&lt;b&gt;, &lt;u&gt;, &lt;i&gt;,&lt;h1&gt;,&lt;h2&gt;,&lt;h3&gt;, and &lt;table&gt; and all associated tags. </td></tr>
	...
	function doAddNews($article, $subject) {
		...
		$article = strip_tags($article, '<a><p><b><u><i><table><td><tr><th><h1><h2><h3>');
		$article = str_replace("\n", "<br>", $article);
		$temp = removeEscapedQuotes($article);
		
		
		$checker = new HTMLSyntaxChecker($temp,"CUSTOM");
		if(!$checker->succeeded()) {
			print "<p>Error: invalid HTML, please check your syntax</p>\n";
			print $checker->htmlError;
			return(0);
		}
	...
	function doNewsPreview($article, $subject, $mode="", $id="") {
		...
		$date = date('d') . '-' . date('m') . '-' . date('y');
		$subject = strip_tags($subject);
		$article = strip_tags($article, '<a><p><br><b><u><i><table><td><tr><th><h1><h2><h3>');
		
		//take care of magic quotes
		$subject = removeEscapedQuotes($subject);
		$article = removeEscapedQuotes($article);
		
		$checker = new HTMLSyntaxChecker($article,"CUSTOM");
		if(!$checker->succeeded()) {
			print "<p>Error: invalid HTML, please check your syntax</p>\n";
			print $checker->htmlError;
			return(0);
	...
	function doViewArticle($id) {
		...
		//replace special entity code with regular ampersand
		//so that the entity character gets displayed properly
